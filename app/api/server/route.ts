export async function POST(req: Request) {
  ...
  try {
    ...
    const res = await fetch(
      'https://www.google.com/recaptcha/api/siteverify?' + new URLSearchParams({secret: secretKey, response: token}).toString(),
      {
        method: "POST",
    });
    ...
    return new Response(JSON.stringify({ success: true, message: "Captcha verified successfully!" }), {
      status: 200,
    });
  } catch (error) {
